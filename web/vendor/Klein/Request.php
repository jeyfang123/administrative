...
class Request
{
     ...
     * @author fushangui
     * @since 2016-11-24
     * @param string product 默认项目名称
     */
    public static function queryProduct($products, $product='property')
    {
        $uri = SELF::createFromGlobals()->pathname();

        $name = @next(explode('/', $uri));
        
        if (!in_array( strtolower($name), $products) && !empty($name))
            return false;
        return empty($name) ? $product : $name ;
    }
}
